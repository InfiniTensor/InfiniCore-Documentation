...
`Equal`, 比较两个张量**所有对应元素**是否**全部**一致。若全部一致，则返回标量true；反之，返回标量false。
...
- `desc`: 使用 `infiniopCreateEqualDescriptor()` 初始化的算子描述符。
- `size`:
  额外空间大小的计算结果的写入地址；
...
```c
infiniopStatus_t infiniopDestroyEqualDescriptor(
    infiniopEqualDescriptor_t desc
);
```

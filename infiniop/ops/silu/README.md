...
`Silu`, 单目逐元素算子。其公式如下:
$${\rm{Silu}}(x) = x \cdot \sigma(x)$$

其中$\sigma(x)$是logistic sigmoid函数。
...
- `output_desc` - { dT | (d1,...,dn) | (...) }:
  算子计算参数 `output` 的张量描述，支持原位计算，支持多向广播。
- `input_desc` - { dT | (d1,...,dn) | (...) }:
...
- `desc`: 使用 `infiniopCreateSiluDescriptor()` 初始化的算子描述符。
- `size`:
  额外空间大小的计算结果的写入地址；
...
```c
infiniopStatus_t infiniopDestroySiluDescriptor(
    infiniopSiluDescriptor_t desc
);
```

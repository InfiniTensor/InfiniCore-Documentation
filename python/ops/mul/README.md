...
逐元素乘法算子，支持广播与非连续张量。定义于 `InfiniCore/python/infinicore/ops/mul.py`。
...
若未提供 `out`，函数会创建并返回新张量；提供 `out` 时将调用 `_infinicore.mul_` 在原地写入。
...
out = ic.mul(a, b)        # 返回新张量
ic.mul(a, b, out=a)       # 原位乘法
```
